...
const TehsilsList = () => {
  ...
  const fetchTehsils = async () => {
    try {
      ...
      let url = '/tehsils';
      const params = [];
      if (districtFilter) params.push(`district_id=${districtFilter}`);
      ...
      setError(err.response?.data?.message || 'Failed to load tehsils');
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => { fetchTehsils(); }, [districtFilter, regionFilter, countryFilter]);

  const handleBack = () => navigate('/dms/geographic/districts/list');
  const handleAdd = () => navigate('/dms/geographic/tehsils/add');
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await axiosInstance.delete(`/tehsils/${tehsilToDelete.id}`);
      setShowDeleteModal(false);
      setTehsilToDelete(null);
      fetchTehsils();
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to delete tehsil');
    }
  };
  ...
  return (
    <>
      ...
      <div className="form-content">
        <PageHeader title="Tehsils" onBack={handleBack} showAdd addPath="/dms/geographic/tehsils/add" />
        {error && <div className="status-message status-message--error">{error}</div>}
        <div className="form-section form-grid-3">
          <FormSelect label="Filter by country" name="countryFilter" value={countryFilter} onChange={(e) => { setCountryFilter(e.target.value); setRegionFilter(''); setDistrictFilter(''); }} options={countryOptions} showDefaultOption defaultOptionText="All countries" />
          <FormSelect label="Filter by region" name="regionFilter" value={regionFilter} onChange={(e) => { setRegionFilter(e.target.value); setDistrictFilter(''); }} options={regionOptions} showDefaultOption defaultOptionText="All regions" />
          <FormSelect label="Filter by district" name="districtFilter" value={districtFilter} onChange={(e) => setDistrictFilter(e.target.value)} options={districtOptions} showDefaultOption defaultOptionText="All districts" />
        </div>
        {loading ? (
          <p>Loading...</p>
        ) : (
      ...
      <ConfirmationModal
        ...
        title="Delete Tehsil"
        message={`Are you sure you want to delete "${tehsilToDelete?.name}"?`}
        onConfirm={handleConfirmDelete}
        onCancel={() => { setShowDeleteModal(false); setTehsilToDelete(null); }}
      />
    </>
  );
};

export default TehsilsList;
