...
const SocialPostEdit = () => {
    ...
    const loadAll = async () => {
      try {
        ...
        const [postRes, chRes] = await Promise.all([
          axiosInstance.get(`/social-posts/${id}`),
          axiosInstance.get('/social-posts/buffer/channels'),
        ]);
        ...
        if (postRes.data?.success) {
          ...
          setForm((prev) => ({
            ...
            buffer_channel_id: p.buffer_channel_id ?? '',
            buffer_channel_name: p.buffer_channel_name ?? '',
          ...
          setError(postRes.data?.message || 'Failed to load post');
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axiosInstance.patch(`/social-posts/${id}`, payload);
      navigate(`/dms/social-posts/view/${id}`);
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to update social post');
    } finally {
    ...
    return (
      <>
        ...
        <div className="form-content">
          <PageHeader title="Edit Social Post" onBack={() => navigate(-1)} />
  ...
  return (
    <>
      <Navbar />
      <div className="form-content">
        <PageHeader title={`Edit Social Post #${id}`} onBack={() => navigate(`/dms/social-posts/view/${id}`)} />
        {error && <div className="status-message status-message--error">{error}</div>}

        <form onSubmit={handleSubmit} className="form">
          <div className="form-section">
            <div className="form-grid-2">
              <FormInput label="Campaign ID (optional)" type="number" name="campaign_id" value={form.campaign_id} onChange={handleChange} />
              <FormInput label="Appeal ID (optional)" type="number" name="appeal_id" value={form.appeal_id} onChange={handleChange} />
            </div>
          </div>
          ...
          <div className="form-section">
            ...
            <FormInput label="Scheduled At (optional)" type="datetime-local" name="scheduled_at" value={form.scheduled_at} onChange={handleChange} />
          </div>

          <div className="form-section">
            <FormTextarea label="Post Text" name="post_text" value={form.post_text} onChange={handleChange} rows={6} />

            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 6, color: overLimit ? '#ef4444' : '#6b7280', fontSize: 13 }}>
              <span>
                {platformLimit ? (
                  <>
                    Characters: <strong>{charCount}</strong> / {platformLimit} {overLimit ? '(over limit for X/Twitter)' : ''}
                  </>
                ) : (
                  <>
              ...
              <button type="button" className="secondary_btn" style={{ padding: '6px 10px' }} onClick={() => navigator.clipboard?.writeText(postText)} disabled={!postText}>
                Copy
              </button>
            </div>
            ...
            <div style={{ marginTop: 16 }}>
              <FormTextarea label="AI Context (optional)" name="ai_context" value={aiContext} onChange={(e) => setAiContext(e.target.value)} rows={3} />
            </div>

            <div className="form-grid-2" style={{ marginTop: 8 }}>
              <FormInput label="Tone" type="text" name="ai_tone" value={aiTone} onChange={(e) => setAiTone(e.target.value)} />
              <FormInput label="Platform" type="text" name="ai_platform" value={aiPlatform} onChange={(e) => setAiPlatform(e.target.value)} />
            </div>
          </div>

          <div className="form-section">
            <FormTextarea label="Image URL (optional)" name="image_url" value={form.image_url} onChange={handleChange} rows={2} />

            <div className="form-actions" style={{ justifyContent: 'space-between' }}>
              <label className="secondary_btn" style={{ cursor: 'pointer', display: 'inline-flex', alignItems: 'center' }}>
                <FiUpload style={{ marginRight: 8 }} />
                Upload Image
                <input type="file" accept="image/*" style={{ display: 'none' }} onChange={(e) => handleUploadImage(e.target.files?.[0])} />
              </label>

              <label style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <input type="checkbox" name="publish_to_buffer" checked={form.publish_to_buffer} onChange={handleChange} />
                Publish to Buffer
              </label>
            </div>
            ...
            {form.image_url ? (
              <div style={{ marginTop: 12 }}>
                <img src={form.image_url} alt="Preview" style={{ maxWidth: '100%', borderRadius: 8 }} />
              </div>
            ) : null}
          </div>

          <div className="form-actions">
            <button type="button" className="secondary_btn" onClick={() => navigate(`/dms/social-posts/view/${id}`)}>
              Cancel
            </button>
            <button type="submit" className="primary_btn" disabled={isSubmitting}>
              {isSubmitting ? 'Saving...' : 'Update Social Post'}
            </button>
          </div>
        </form>
      </div>
