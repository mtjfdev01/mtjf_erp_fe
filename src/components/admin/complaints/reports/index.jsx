...
import { getComplaintPermissions, isSuperAdmin } from '../../../../utils/permissions';
...
function createOrUpdateDoughnutChart(ctx, data, chartInstanceRef, chartSizes, isMobile) {
  const drawLabels = (chart) => {
    ...
    if (total > 0) {
      ...
      ctx.fillText('Total Complaints', centerX, centerY + 14);
      ctx.restore();
    }
  };
    ...
    <div className="task-report-card task-report-card--project-report">
      <div className="task-report-card-header task-report-card-header--with-filter" ref={headerRef}>
        <div className="task-report-header-left">
          ...
          <h2 className="task-report-card-title">Project/Program-wise Complaint Report</h2>
        </div>
...
const TaskReports = () => {
  ...
  const [viewType, setViewType] = useState("assigned"); // Initial view: current user's assigned complaints
  ...
  /** Complaint dashboard scope: user's department (permissions resolved against it). */
  const complaintsDepartmentFromUser = useMemo(() => {
    const d = String(user?.department || '').trim().toLowerCase();
    return d;
  }, [user?.department]);

  const taskPerms = useMemo(
    () => getComplaintPermissions(permissions || {}, complaintsDepartmentFromUser || user?.department, user?.role),
    [permissions, user?.department, user?.role, complaintsDepartmentFromUser],
  );
  ...
  const taskScopeCaps = useMemo(() => {
    ...
    return {
      ...
      canViewTeamToggle,
      availableViewTypes,
      // For a pure "User" / self-scope role, default "All Complaints" must mean "All of YOUR complaints"
  ...
  const isGeneralAdminDashboard = useMemo(() => {
    ...
    return (
      ...
      r === 'super_admin' ||
      complaintsDepartmentFromUser === 'admin'
    );
  }, [user?.role, permissions, complaintsDepartmentFromUser]);
  ...
  const fetchTaskReports = useCallback(async () => {
    ...
    try {
      ...
      if (isGeneralAdminDashboard) {
        ...
        statsDepartment = complaintsDepartmentFromUser;
      ...
      const statsRes = await axiosInstance.get('/tickets/dashboard/stats', { params: statsParams });
      ...
      const reportsRes = await axiosInstance.get('/tickets/reports', { params: reportsParams });
      ...
      console.error('Complaint reports fetch error:', e);
      setTaskStatsError(e.response?.data?.message || e.message || 'Failed to fetch task reports');
    } finally {
      setTaskStatsLoading(false);
    }
  }, [duration, selectedDepartment, rolePerms.scope, rolePerms.isAdmin, user, user?.department, user?.id, complaintsDepartmentFromUser, isGeneralAdminDashboard, permissions, viewType, showTeamPerformance, taskScopeCaps.isSelfOnly, getDateRangeForDuration]);
  ...
  useEffect(() => {
    ...
    if (filteredUserReportUsers.length > 0 && userBarChartRef.current) {
      ...
      // Calculate total complaints per status across all users
      const statusTotals = STATUS_LABELS.map((statusLabel, index) => {
        const statusKey = statusLabel.toLowerCase().replace(/\s+/g, '_');
      ...
      // Filter out statuses with zero complaints
      const activeStatuses = statusTotals.filter(item => item.total > 0);
  ...
  const filterElement = (
    <div className="task-dashboard-filters">
      ...
      {taskScopeCaps.showTasksDropdown && (
        <div className="task-filter-group">
          <span className="task-filter-label">Complaints</span>
          <select
            className="task-filter-select"
            value={viewType}
          ...
          >
            ...
            <option value="approval_tasks">Approval Complaints</option>
          </select>
        </div>
      )}
  ...
  return (
    <>
      ...
      <div className="task-report-container">
        <PageHeader title="Tickets Dashboard" showBackButton={true} rightElement={filterElement} />
              ...
              <>
                <div className="task-dashboard-header">
                  ...
                  <div className="task-dashboard-header-top">
                    ...
                    <div className="task-dashboard-cards">
                        <div className="task-stat-card task-stat-card--total">
                          ...
                          <div className="task-stat-label">Total Complaints</div>
                        ...
                        <div className="task-stat-card task-stat-card--overdue">
                          ...
                          <div className="task-stat-label">Overdue Complaints</div>
                          <div className="task-stat-value">
                  ...
                  <div className="task-dashboard-main">
                    ...
                    <div className="task-dashboard-column">
                      ...
                      <div className="task-report-card task-report-card--task-progress">
                        ...
                        {taskStatsLoading && <div className="loading">Loading ticket progress...</div>}
                  ...
                  <div className="task-dashboard-reports-grid">
                    {/* Project/Program-wise Complaint Report */}
                    ...
                    {/* <div className="task-report-card task-report-card--user-report">
                      <div className="task-report-card-header task-report-card-header--with-filter" ref={userReportHeaderRef}>
                        <div className="task-report-header-left">
                          ...
                          <h2 className="task-report-card-title">User-wise Complaint Report</h2>
                        </div>
                    ...
                    {/* Department-wise Complaint Report (Only for Admin) */}
                    {/* {rolePerms.isAdmin && (
                      <div className="task-report-card task-report-card--department-report">
                        <div className="task-report-card-header task-report-card-header--with-filter" ref={deptReportHeaderRef}>
                          <div className="task-report-header-left">
                            ...
                            <h2 className="task-report-card-title">Department-wise Complaint Report</h2>
                          </div>
