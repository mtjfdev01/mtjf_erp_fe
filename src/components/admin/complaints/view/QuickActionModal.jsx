...
const QuickActionModal = ({
    ...
    try {
      ...
      if (actionKey === 'REASSIGN') {
        ...
        const name =
          ...
          primary.email ||
          `User #${primary.id}`;
        const res = await axiosInstance.post(`/tickets/${taskId}/reassign`, {
          assignee_name: name,
          assigned_users: ids,
        ...
        await axiosInstance.patch(`/tickets/${taskId}`, payload);
        ...
        await axiosInstance.post(`/tickets/${taskId}/comments`, {
          content,
        });
      ...
      if (!updated) {
        try {
          const refreshed = await axiosInstance.get(`/tickets/${taskId}`);
  ...
  const title = QUICK_ACTION_LABEL_MAP[actionKey] || 'Ticket action';
