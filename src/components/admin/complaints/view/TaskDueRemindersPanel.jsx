...
const TaskDueRemindersPanel = ({ taskId, dueDate, isAssignee, disabled }) => {
  ...
  const loadReminders = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await axiosInstance.get(`/tickets/${taskId}/due-reminders`);
  ...
  const handleAdd = async (e) => {
    e.preventDefault();
    if (!hasDueDate) {
      toast.error('Set a due date on this ticket first.');
      return;
    }
    if (!remindOnDate) {
    ...
    setSaving(true);
    try {
      await axiosInstance.post(`/tickets/${taskId}/due-reminders`, {
  ...
  const handleDelete = async (reminderId) => {
    try {
      await axiosInstance.delete(`/tickets/${taskId}/due-reminders/${reminderId}`);
    ...
    <div className="view-section task-due-reminders-panel">
      ...
      <p className="task-due-reminders-note">
        ...
        {hasDueDate && (
          <>
            {' '}
            Ticket due: <strong>{formatRemindDate(dueDate)}</strong>
          </>
        )}
      </p>
      ...
      {!hasDueDate && (
        <div className="task-due-reminders-empty">
          Add a due date to this ticket before setting reminders.
        </div>
      )}
