...
const AddTask = ({
  ...
  const kindLabel = form.type === 'complaint' ? 'Complaint' : 'Issue';
  const kindLower = kindLabel.toLowerCase();
  const createTitle = taskPerms.canCreate
    ? `Create ${kindLabel}`
    : `You do not have permission to create ${kindLower}s`;
  ...
  const handleBack = useCallback(() => {
    ...
    navigate(`${complaintsBasePath()}/list`); // Navigate back to tickets list at /tickets/list
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.title || !form.title.trim()) {
      validationErrors.push(`${kindLabel} title is required.`);
    }
    ...
    if (!form.task_type) {
      validationErrors.push('Schedule is required.');
    }
    ...
    if (!form.scope) {
      validationErrors.push('Scope is required.');
    }
    if (!form.start_date) {
    ...
    if (movItemsCleanForValidation.length === 0) {
      validationErrors.push(
        `At least one Means of Verification (MOV) item is required for every ${kindLower}.`
      );
    }
    if (form.workflow_type === 'approval_required') {
      if (!approverUsers || approverUsers.length === 0) {
        validationErrors.push(`At least one approver is required for approval-required ${kindLower}s.`);
      }
    }
    if (form.task_type === 'recurring') {
      if (!form.recurrence_frequency) {
        validationErrors.push(`Recurring frequency is required for recurring ${kindLower}s.`);
      }
    }
    ...
    try {
      ...
      toast.success(`${kindLabel} created. Assignment emails will send if configured.`);
      ...
      if (createdTaskId) {
        ...
        if (toUpload.length > 0) {
          ...
          if (failed > 0) {
            toast.error(
              failed === 1
                ? `${kindLabel} created, but failed to upload 1 attachment.`
                : `${kindLabel} created, but failed to upload ${failed} attachments.`,
            );
          }
        }
      }
    } catch (e2) {
      setError(e2.response?.data?.message || `Failed to create ${kindLower}.`);
      toast.error(e2.response?.data?.message || `Failed to create ${kindLower}.`);
    } finally {
      ...
      if (createdTaskId) {
        navigate(`${complaintsBasePath()}/view/${createdTaskId}`, { replace: true }); // Replace history entry so back goes to list
      }
    }
  };
        ...
        <div className={`add-task-card${isModal ? ' add-task-card--modal' : ''}`}>
          {isModal ? (
            <div className="task-form-modal-header">
              <h2 className="task-form-modal-title">Add {kindLabel}</h2>
              <button
                type="button"
            ...
            <PageHeader title={`Add ${kindLabel}`} showBackButton={true} onBackClick={handleBack} />
          )}
          {error && <div className="status-message status-message--error">{error}</div>}
          {submitting && (
            <div className="add-task-submitting-overlay">
              Creating {kindLower}, please wait...
            </div>
          )}
          <form onSubmit={handleSubmit}>
            <div className="add-task-section add-task-section--compact">
              ...
              <div className="add-task-grid-2">
                ...
                <FormInput
                  ...
                  placeholder={`Enter ${kindLower} title`}
                  required
                />
              </div>
              <div className="add-task-description-wrap">
                <FormTextarea
                  ...
                  onChange={handleChange}
                  placeholder={`Enter a brief description of the ${kindLower}...`}
                  rows={3}
                  maxLength={500}
                  required
                />
            ...
            <div className="add-task-section add-task-section--compact" style={{ marginBottom: '0.85rem' }}>
              <div className="add-task-section-title">2. {kindLabel} Setup</div>
              <div className="add-task-setup-grid">
                ...
                <TaskFormSelect
                  ...
                  iconClassName="task-custom-select-field-icon--task-type"
                  options={[
                    { value: 'one_time', label: `One-time ${kindLower}` },
                    { value: 'recurring', label: `Recurring ${kindLower}` },
                    { value: 'project_linked', label: `Project-linked ${kindLower}` }
                  ]}
                  required
                />
              </div>
            ...
            <div className="add-task-split-row">
              <div className="add-task-section add-task-section--compact add-task-section--schedule">
                ...
                <div className="add-task-grid-1">
                  ...
                  <FormInput
                    name="due_date"
                    label={form.task_type === 'recurring' ? `Due Date (of first ${kindLower}) - Auto-calculated` : 'Due Date'}
                    type="date"
                    value={form.due_date}
                    onChange={handleChange}
            ...
            <div className="add-task-footer add-task-footer--actions">
              ...
              >
                <FiPlus />
                {submitting ? 'Submitting....' : `Create ${kindLabel}`}
              </button>
            </div>
          </form>
        </div>
