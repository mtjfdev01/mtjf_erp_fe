...
const UpdateTask = ({
  ...
  const handleBack = useCallback(() => {
    ...
    navigate(`${taskRouteBase}/list`); // Navigate to tickets list at /tickets/list instead of previous page
  ...
  const editTitle =
    String(form.status).toLowerCase() === 'completed' && !canEditCompleted
      ? `Your role cannot edit completed ${kindLower}s`
      : 'Save Changes';
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      setError('');
      try {
        const res = await axiosInstance.get(`/tickets/${id}?include_all_mov=true`);
        ...
        setForm({
          ...
          task_type: t.complaint_type || t.task_type || '',
        ...
        setError(e.response?.data?.message || `Failed to load ${kindLower}.`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axiosInstance.patch(`/tickets/${id}`, payload);
      const updatedTask = res?.data?.data || null;
      toast.success(`${kindLabel} updated. Email notification will be sent if configured.`);
      ...
      if (toUpload.length > 0) {
        ...
        if (failed > 0) {
          toast.error(
            failed === 1
              ? `${kindLabel} updated, but failed to upload 1 attachment.`
              : `${kindLabel} updated, but failed to upload ${failed} attachments.`,
          );
        }
      }
      ...
      setError(e2.response?.data?.message || `Failed to update ${kindLower}.`);
      toast.error(e2.response?.data?.message || `Failed to update ${kindLower}.`);
    } finally {
      setSaving(false);
    }
  };
          ...
          <div className={`add-task-card${isModal ? ' add-task-card--modal' : ''}`}>
            {isModal ? (
              <div className="task-form-modal-header">
                <h2 className="task-form-modal-title">Update {kindLabel}</h2>
                <button
                  type="button"
              ...
              <PageHeader title={`Update ${kindLabel}`} showBackButton={true} onBackClick={handleBack} />
            )}
            <div className="status-message">Loading {kindLower}...</div>
          </div>
        </div>
      </>
    );
