...
const ComplaintsList = ({ viewMode = 'kanban', onViewModeChange, refreshNonce = 0 }) => {
  ...
  const tasksRouteBase = useMemo(() => complaintsBasePath(), []);
  ...
  const hoverText = (action) => {
    if (action === 'create' && !taskPerms.canCreate) return 'You do not have permission to create tickets';
    if (action === 'view' && !taskPerms.canViewDetail) return 'You do not have permission to view tickets';
    if (action === 'update' && !taskPerms.canUpdate) return 'You do not have permission to edit tickets';
    if (action === 'delete' && !taskPerms.canDelete) return 'You do not have permission to delete tickets';
    if (action === 'assign' && !taskPerms.canAssign) return 'You do not have permission to assign tickets';
    if (action === 'approve' && !taskPerms.canApprove) return 'You do not have permission to approve tickets';
    if (action === 'complete' && !taskPerms.canComplete) return 'You do not have permission to complete tickets';
    if (action === 'edit_completed' && !taskPerms.canEditCompleted) return 'You do not have permission to edit completed tickets';
  ...
  const fetchApprovals = useCallback(async () => {
    ...
    let cancelled = false;
    try {
      const res = await axiosInstance.get('/tickets/approvals/my');
  ...
  const approvalRequestsForUser = useMemo(() => {
    ...
    myApprovals.forEach((approval) => {
      ...
      // Use the ticket object attached directly to the approval record
      ...
      // Include complaints where:
      // 1. Complaint is NOT fully approved/closed AND status is pending_approval OR rejected AND user hasn't acted yet
  ...
  const handleReassignSubmit = async () => {
    ...
    setReassignError('');
    try {
      const res = await axiosInstance.post(`/tickets/${reassignTask.id}/reassign`, {
        assignee_name: name,
        assigned_users: ids,
      ...
      const msg = e.response?.data?.message || 'Failed to reassign ticket.';
  ...
  const handleQuickStatusChange = async (task, nextStatus) => {
    ...
    if (!canUpdate && !canChangeAsAssignee) {
      toast.error(hoverText('update') || 'You do not have permission to update tickets');
      return;
    }
    ...
    try {
      ...
      const res = await axiosInstance.post(
        `/tickets/${task.id}/status-transition`,
        payload
      );
      const updated = res.data?.data || null;
      if (updated) {
    ...
    try {
      await Promise.all(
        selectedTaskIds.map((id) =>
          axiosInstance
            .post(`/tickets/${id}/status-transition`, { status: nextStatus, notes: '' })
            .catch(() => null)
        )
      );
      setTasks((prev) =>
                ...
                <div className="tl-empty-tab-state">
                  ...
                  {activeTab === 'assigned_to_team' && (
                    <>
                      ...
                      <p>No tickets assigned to your team members.</p>
                    </>
                  )}
                  ...
                  {activeTab === 'other_tasks' && (
                    <>
                      ...
                      <p>No tickets assigned by you found.</p>
                    </>
                  )}
                </div>
              )}
