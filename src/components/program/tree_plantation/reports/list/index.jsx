...
const TreePlantationReportsList = () => {
  ...
  const fetchReports = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get('/program/tree_plantation/reports', {
        params: {
          page: currentPage,
          pageSize: pageSize,
          sortField: sortField,
  ...
  const handleEdit = (id) => navigate(`/program/tree_plantation/reports/update/${id}`);
  const handleView = (id) => navigate(`/program/tree_plantation/reports/view/${id}`);
  const handleDeleteClick = (report) => { setReportToDelete(report); setShowDeleteModal(true); };
  const handleDeleteConfirm = async () => {
    if (reportToDelete) {
      try {
        await axiosInstance.delete(`/program/tree_plantation/reports/${reportToDelete.id}`);
        fetchReports();
        setError('');
      } catch (err) {
    ...
    <>
      ...
      <div className="list-wrapper">
        <div className="list-content">
          <PageHeader 
            title="Tree Plantation Reports" 
            showBackButton={false}
            showAdd={true}
            addPath="/program/tree_plantation/reports/add"
          />
          {error && <div className="status-message status-message--error">{error}</div>}
          {reports.length === 0 ? (
            <div className="empty-state">
              <div className="empty-state-text">No tree plantation reports found</div>
      ...
      <ConfirmationModal
        ...
        text={`Are you sure you want to delete the tree plantation report from ${reportToDelete?.report_date ? formatDate(reportToDelete.report_date) : ''}?`}
        delete={true}
...
export default TreePlantationReportsList; 